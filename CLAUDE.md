...
- **Data fetching** (all cached in `localStorage` with TTLs from `CONFIG.TTL`):
  - `fetchRace()` — next upcoming race from the Jolpica F1 API (Ergast-compatible), cached 8h.
...
- Worker (`worker/src/index.js`): `fetch()` serves `/subscribe` and `/unsubscribe`, storing subscriptions in the `SUBSCRIPTIONS` KV namespace keyed by a SHA-256 hash of the endpoint. `scheduled()` runs every 5 minutes but only reads/writes KV on most ticks — `getNextRace()` caches the next race in the `RACE_CACHE` KV namespace with an 8h TTL (matching the client's own `CONFIG.TTL.RACE`), so Jolpica only actually gets hit a few times a day, not on every cron tick. For each rule in `NOTIFY_RULES` it checks whether "now" just crossed `sessionTime - leadMs`; if so it sends to every stored subscription via `web-push` (using the `nodejs_compat` flag) and records a dedupe flag in the `SENT` KV namespace (`{season}-{round}:{tag}`, 7-day TTL) so overlapping cron ticks don't double-send. A 404/410 from a push send means the browser dropped that subscription, so the Worker deletes it from `SUBSCRIPTIONS`.
